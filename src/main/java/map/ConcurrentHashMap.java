...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
     ...
     * 计算出来的结果是65536 表示并发扩容最多能达到的线程数 基本达不到这么大
     */
     ...
     */
    //当node节点的hash值 为-1 时，表示当前节点是FWD节点
    static final int MOVED     = -1; // hash for forwarding nodes
    //当node节点的hash值 为-2时 表示当前节点已经树化，且当前节点为TreeBin对象，TreeBin对象代理操作红黑树
    static final int TREEBIN   = -2; // hash for roots of trees
    static final int RESERVED  = -3; // hash for transient reservations
    //31个1  0111 1111 1111 1111 1111 1111 1111 1111   可以将一个负数通过位与运算后可以得到正数，但并不是取绝对值
    ...
    static {
        try {
            ...
            //1 0000 & 0 1111 = 0
            if ((scale & (scale - 1)) != 0) //保证2的次方数  & 只有两个 都为1 时其结果才为 1
                throw new Error("data type scale not a power of two");
            //numberOfLeadingZeros() 这个方法是返回当前数值转换为二进制后，从高位到低位开始统计，看有多少个0连续在一块。
            //8 => 1000 numberOfLeadingZeros(8) = 28  （Integer 占用32位 32-4  = 28）
            //4 => 100 numberOfLeadingZeros(4) = 29
            //ASHIFT = 31 - 29 = 2 ？？
            //ABASE + （5 << ASHIFT）  ==> 相当于 ABASE + 5 * 4
